...
export default function Employees({ employees, addEmployee, updateEmployee, deleteEmployee, currency }) {
  ...
  const save = async () => {
    if (modal === "add") await addEmployee({ ...f, salary: +f.salary });
    else await updateEmployee(modal, { ...f, salary: +f.salary });
    setModal(null);
  };
  const del = async (id) => { await deleteEmployee(id); setConfirm(null); };
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
        ...
        <div style={S.row}>
          <span style={{ fontSize: 12, color: C.textMid }}>Payroll: <b style={{ color: C.text }}>{fmtMoney(totalSalary, currency)}/mo</b></span>
      ...
      <div style={{ backgroundColor: C.cardGreen, borderRadius: 12, overflow: "hidden" }}>
        ...
        {employees.map((emp) => (
          <div key={emp._id} style={{ display: "grid", gridTemplateColumns: "2fr 1.5fr 1fr 1fr .8fr 90px", padding: "12px 16px", backgroundColor: C.card, borderTop: `1px solid ${C.cardInner}`, alignItems: "center" }}>
            <div>
            ...
            <div style={{ fontSize: 12, fontWeight: "bold", color: C.text }}>{fmtMoney(emp.salary, currency)}<span style={{ fontWeight: "normal", fontSize: 10, opacity: 0.5 }}>/mo</span></div>
            ...
            <div style={S.row}>
              <button style={{ ...S.btn(C.btnGreenL), padding: "4px 9px", fontSize: 11, marginLeft: 9 }} onClick={() => { setF({ ...emp }); setModal(emp._id); }}>✏️</button>
              <button style={{ ...S.btn(C.btnRed),    padding: "4px 9px", fontSize: 11 }} onClick={() => setConfirm(emp._id)}>🗑</button>
            </div>
          </div>
        ))}
      </div>
      ...
      {modal && (
        <Modal title={modal === "add" ? "New Employee" : "Edit Employee"} onClose={() => setModal(null)}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            ...
            <FF label={`Monthly Salary (${currency})`}><input style={S.input} type="number" value={f.salary} onChange={set("salary")} /></FF>
            <FF label="Status">
