...
export default function Finances({ transactions, addTransaction, updateTransaction, deleteTransaction, currency }) {
  const [modal, setModal]     = useState(null);
  const [confirm, setConfirm] = useState(null);
  const [filter, setFilter]   = useState("All");
  const def = { date: new Date().toISOString().slice(0,10), label: "", type: "Expense", amount: 0, category: "Feed" };
  const [f, setF] = useState(def);
  const set = (k) => (e) => setF((p) => ({ ...p, [k]: e.target.value }));

  const save = async () => {
    if (modal === "add") await addTransaction({ ...f, amount: +f.amount });
    else await updateTransaction(modal, { ...f, amount: +f.amount });
    setModal(null);
  };
  const del = async (id) => { await deleteTransaction(id); setConfirm(null); };
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 16, marginBottom: 22 }}>
        {[["Revenue", income, "#5A8A6A"], ["Expenses", expense, C.btnRed], ["Balance", income - expense, income - expense >= 0 ? "#5A8A6A" : C.btnRed]].map(([l, v, c]) => (
          <div key={l} style={{ backgroundColor: C.cardGreen, borderRadius: 12, padding: "18px 20px" }}>
            ...
            <div style={{ fontSize: 24, fontWeight: "bold", color: c }}>{fmtMoney(v, currency)}</div>
          </div>
        ))}
      </div>
      ...
      {filtered.map((t) => (
        <div key={t._id} style={{ display: "flex", alignItems: "center", gap: 12, backgroundColor: C.card, borderRadius: 8, padding: "10px 16px", marginBottom: 8 }}>
          ...
          <button style={S.btn(C.btnGreenL)} onClick={() => { setF({ ...t }); setModal(t._id); }}>✏️</button>
          <button style={S.btn(C.btnRed)}    onClick={() => setConfirm(t._id)}>🗑</button>
        </div>
      ))}
      ...
      {modal && (
        <Modal title={modal === "add" ? "New Transaction" : "Edit Transaction"} onClose={() => setModal(null)}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            ...
            <FF label={`Amount (${currency})`}><input style={S.input} type="number" value={f.amount} onChange={set("amount")} /></FF>
            <FF label="Category">
