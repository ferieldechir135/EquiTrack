...
export default function Inventory({ inventory, addInventory, updateInventory, deleteInventory }) {
  ...
  const save = async () => {
    if (modal === "add") await addInventory({ ...f, qty: +f.qty, maxQty: +f.maxQty });
    else await updateInventory(modal, { ...f, qty: +f.qty, maxQty: +f.maxQty });
    setModal(null);
  };
  const del = async (id) => { await deleteInventory(id); setConfirm(null); };
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
        ...
        <button style={ S.btn() } onClick={() => { setF(def); setModal("add"); }}>+Add item</button>
      </div>
        ...
        {groups.map(({ cat, items }) => (
          <div key={cat} style={{ backgroundColor: C.card, borderRadius: 12, overflow: "hidden" }}>
            ...
            {items.map((item) => {
              ...
              const isLow = pct < 25;
              return (
                <div key={item._id} style={{ padding: "10px 14px", borderTop: `1px solid ${C.cardInner}` }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        ...
                        <div style={S.row}>
                          <button style={{ ...S.btn(C.btnGreenL), padding: "3px 8px", fontSize: 9 }} onClick={() => { setF({ ...item }); setModal(item._id); }}>✏️</button>
                          <button style={{ ...S.btn(C.btnRed),    padding: "3px 8px", fontSize: 9 }} onClick={() => setConfirm(item._id)}>🗑</button>
                        </div>
                      </div>
