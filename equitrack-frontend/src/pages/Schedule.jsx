...
import { fmtDate, evtColor, MONTHS } from "../utils/helpers";
import { Modal, Confirm, FF, Badge } from "../components/UI";

export default function Schedule({ events, addEvent, updateEvent, deleteEvent, horses, riders }) {
  ...
  const save     = async () => { if (modal === "add") await addEvent(f); else await updateEvent(modal, f); setModal(null); };
  const del      = async (id) => { await deleteEvent(id); setConfirm(null); };
  const openEdit = (ev) => { setF({ ...ev }); setModal(ev._id); };
  const openAdd  = (date = "") => { setF({ ...def, date }); setModal("add"); };

  const vpill = (active) => ({ padding: "7px 18px", borderRadius: active ? 24 : 20, border: "none", cursor: "pointer", fontSize: 12, backgroundColor: active ? C.cardGreen : C.card, color: active ? "#fff" : C.text });
    ...
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
        <div style={{ display: "flex", gap: 8 }}>
          {["Monthly","Weekly","Daily"].map((v) => <button key={v} style={vpill(viewMode === v)} onClick={() => setViewMode(v)}>{v} view</button>)}
        </div>
        <button style={{ ...S.btn(C.cardGreen), borderRadius: 8, padding: "7px 18px", fontSize: 12 }} onClick={() => openAdd()}>+New event</button>
      </div>
      ...
      <div style={{ backgroundColor: C.cardGreen, borderRadius: 12, overflow: "hidden", marginBottom: 16 }}>
        <div style={{ display: "flex", alignItems: "center", padding: "10px 18px", gap: 8 }}>
          ...
          <button style={{ ...S.btn(C.card, C.text), borderRadius: 8, padding: "4px 12px" }} onClick={() => setCurDate(new Date(yr, mo + 1, 1))}>{">"}</button>
          <span style={{ flex: 1, textAlign: "center", color: "#fff", fontSize: 18, letterSpacing: 4, fontWeight: "bold" }}>{MONTHS[mo].toUpperCase()} {yr}</span>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", backgroundColor: C.card }}>
          {["SUN","MON","TUE","WED","THU","FRI","SAT"].map((d) => <div key={d} style={{ textAlign: "center", padding: "8px 0", fontSize: 9, letterSpacing: 2, fontWeight: "bold", color: C.text }}>{d}</div>)}
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 1, backgroundColor: C.cardInner }}>
          {cells.map((day, i) => (
            <div key={i} onClick={() => day && openAdd(dayStr(day))} style={{ minHeight: 72, backgroundColor: C.pageBg, padding: "5px 6px", cursor: day ? "pointer" : "default" }}>
              {day && (
                <>
                  <div style={{ fontSize: 11, color: C.textFaint, marginBottom: 3 }}>{day}</div>
                  {evFor(day).map((ev) => (
                    <div key={ev._id} onClick={(e) => { e.stopPropagation(); openEdit(ev); }}
      ...
      {viewMode !== "Monthly" && (
        <div style={{ marginBottom: 14 }}>
          ...
          {[...moEvts].sort((a, b) => a.date.localeCompare(b.date)).map((ev) => (
            <div key={ev._id} style={{ display: "flex", alignItems: "center", gap: 12, backgroundColor: C.card, borderRadius: 8, padding: "10px 14px", marginBottom: 8 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 10, color: C.textFaint }}>{fmtDate(ev.date)}{ev.horse && ` · 🐴 ${ev.horse}`}{ev.rider && ` · 👤 ${ev.rider}`}</div>
              </div>
              <Badge label={ev.type} color={evtColor(ev.type)} />
              <button style={S.btn(C.btnGreenL)} onClick={() => openEdit(ev)}>✏️</button>
              <button style={S.btn(C.btnRed)}    onClick={() => setConfirm(ev._id)}>🗑</button>
            </div>
          ))}
        </div>
      )}
      ...
      <div style={{ backgroundColor: C.card, borderRadius: 10, padding: "14px 20px" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <span style={{ fontSize: 11, letterSpacing: 2, textTransform: "uppercase", fontWeight: "bold", color: C.text }}>Today's Plan – {new Date().toLocaleDateString("en-GB", { day: "numeric", month: "long" })}</span>
          <span style={{ color: C.gold, fontSize: 16 }}>∨</span>
        </div>
        {todayEvts.length === 0 ? <div style={{ fontSize: 12, color: C.textFaint, marginTop: 8 }}>No events today.</div> : todayEvts.map((ev) => (
          <div key={ev._id} style={{ fontSize: 12, color: C.text, padding: "6px 0", borderBottom: `1px solid ${C.cardInner}`, marginTop: 4 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            ...
            <FF label="Rider">
              <select style={S.input} value={f.rider} onChange={set("rider")}>
                <option value="">– None –</option>
                {riders.map((r) => <option key={r._id}>{r.name}</option>)}
              </select>
            </FF>
