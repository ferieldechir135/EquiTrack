...
export default function Riders({ riders, addRider, updateRider, deleteRider, horses }) {
  ...
  const save = async () => {
    if (modal === "add") await addRider(f);
    else await updateRider(modal, f);
    setModal(null);
  };
  const del = async (id) => { await deleteRider(id); setConfirm(null); };
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
        ...
        <div style={S.row}>
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search rider…" style={{ ...S.input, borderRadius: 20, width: 200 }} />
          <button style={S.btn()} onClick={() => { setF(def); setModal("add"); }}>+ New rider</button>
        </div>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 14 }}>
        {filtered.map((r) => (
          <div key={r._id} style={{ backgroundColor: C.card, borderRadius: 12, padding: 18 }}>
            ...
            {[["🐴", r.horse || "–"], ["📞", r.phone || "–"], ["✉️", r.email || "–"], ["📅", fmtDate(r.joinDate)]].map(([ic, v]) => (
              <div key={ic} style={{ fontSize: 12, color: C.text, marginBottom: 5 }}><span style={{ opacity: 0.5 }}>{ic} </span>{v}</div>
            ))}
      ...
      {modal && (
        <Modal title={modal === "add" ? "New Rider" : "Edit Rider"} onClose={() => setModal(null)}>
          ...
          <FF label="Notes"><textarea style={{ ...S.input, height: 60, resize: "vertical" }} value={f.notes} onChange={set("notes")} /></FF>
