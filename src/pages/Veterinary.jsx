...
export default function Veterinary({ vetRecords, setVetRecords, horses }) {
  ...
  const save = () => {
    if (modal === "add") setVetRecords((rs) => [...rs, { ...f, id: uid() }]);
    else setVetRecords((rs) => rs.map((r) => r.id === modal ? { ...r, ...f } : r));
    setModal(null);
  };
  const del   = (id) => { setVetRecords((rs) => rs.filter((r) => r.id !== id)); setConfirm(null); };
  ...
  return (
    <div>
      ...
      {[...vetRecords].sort((a, b) => b.date.localeCompare(a.date)).map((r) => (
        <div key={r.id} style={{ backgroundColor: C.card, borderRadius: 10, padding: "14px 18px", marginBottom: 10 }}>
