...
import { uid, fmtDate, stateColor } from "../utils/helpers";
  ...
  return (
    <>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <FF label="Nom"><input style={S.input} value={f.name} onChange={set("name")} /></FF>
        <FF label="État">
          <select style={S.input} value={f.state} onChange={set("state")}>
            {["HEALTHY", "INJURY", "SICK", "REST"].map((s) => <option key={s}>{s}</option>)}
          </select>
        </FF>
        <FF label="Race"><input style={S.input} value={f.breed} onChange={set("breed")} /></FF>
        <FF label="Âge (ans)"><input style={S.input} type="number" value={f.age} onChange={set("age")} /></FF>
        <FF label="Box"><input style={S.input} value={f.stall} onChange={set("stall")} /></FF>
        <FF label="Propriétaire"><input style={S.input} value={f.owner} onChange={set("owner")} /></FF>
        <FF label="Sexe">
        ...
        <FF label="Date d'arrivée"><input style={S.input} type="date" value={f.arrival} onChange={set("arrival")} /></FF>
        <FF label="Entraînement"><input style={S.input} value={f.training} onChange={set("training")} /></FF>
      </div>
      <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 4 }}>
        <button style={S.btn(C.btnBeige, C.text)} onClick={onClose}>Annuler</button>
        <button style={S.btn()} onClick={() => f.name && onSave(f)}>Enregistrer</button>
      </div>
    </>
  );
}
...
function HorseCard({ horse, onEdit, onDelete, onToggle, onAddHistory }) {
  ...
  const pill = (bg, col = "#fff") => ({
    backgroundColor: bg, color: col, border: "none", borderRadius: 20,
    padding: "5px 16px", fontSize: 11, cursor: "pointer", letterSpacing: 0.5,
  });
  ...
  return (
    <div style={{ borderRadius: 12, marginBottom: 12, overflow: "hidden", boxShadow: "0 2px 10px rgba(0,0,0,.1)" }}>
      ...
      <div
        onClick={onToggle}
        style={{ backgroundColor: C.cardGreen, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 20px", cursor: "pointer" }}
      >
        <div style={S.row}>
          <span style={{ fontSize: 18, fontWeight: "bold", color: "#fff", letterSpacing: 0.5 }}>{horse.name}</span>
      ...
      {horse.open && (
        <div style={{ backgroundColor: C.card, padding: "18px 20px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 10, marginBottom: 10 }}>
            {[
              ["BREED", horse.breed], ["AGE", horse.age + " yrs"], ["STALL", horse.stall], ["OWNER", horse.owner],
              ["SEX", horse.sex], ["VACCINATION", horse.vaccination], ["ARRIVAL", fmtDate(horse.arrival)], ["TRAINING", horse.training],
            ].map(([l, v]) => (
          ...
          <div style={{ marginTop: 10 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
              <span style={{ fontSize: 11, fontWeight: "bold", letterSpacing: 1.5, textTransform: "uppercase", color: C.text }}>
                Medical History :
              </span>
            ...
            {showHF && (
              <div style={{ backgroundColor: C.white, borderRadius: 8, padding: 14, marginBottom: 10 }}>
                ...
                <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
                  <button style={S.btn(C.btnBeige, C.text)} onClick={() => setShowHF(false)}>Annuler</button>
                  <button style={S.btn()} onClick={() => { onAddHistory(horse.id, { ...hf, id: uid() }); setShowHF(false); setHf({ date: "", type: "", doctor: "", clinic: "" }); }}>
                    Ajouter
                  </button>
                </div>
              </div>
            ...
            {horse.history.length === 0 ? (
              ...
              [...horse.history].sort((a, b) => b.date.localeCompare(a.date)).map((h, i) => (
                <div key={h.id} style={{ display: "flex", gap: 10, marginBottom: 10 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 10, letterSpacing: 0.5, color: C.textFaint }}>
                      DOCTOR: {h.doctor} &nbsp; CLINIC: {h.clinic}
                    </div>
                  </div>
                </div>
              ))
...
// ─── Horses Page ─────────────────────────────────────────────
export default function Horses({ horses, setHorses }) {
  const [search, setSearch]   = useState("");
  const [modal, setModal]     = useState(null);
  const [confirm, setConfirm] = useState(null);

  const toggle     = (id) => setHorses((hs) => hs.map((h) => h.id === id ? { ...h, open: !h.open } : h));
  const save       = (f) => { if (modal.type === "add") setHorses((hs) => [...hs, { ...f, id: uid(), open: false }]); else setHorses((hs) => hs.map((h) => h.id === modal.horse.id ? { ...h, ...f } : h)); setModal(null); };
  const del        = (id) => { setHorses((hs) => hs.filter((h) => h.id !== id)); setConfirm(null); };
  const addHistory = (hid, rec) => setHorses((hs) => hs.map((h) => h.id === hid ? { ...h, history: [...h.history, rec] } : h));
  ...
  return (
    <div>
        ...
        <div style={S.row}>
          <div style={{ position: "relative" }}>
            ...
            <input
              ...
              placeholder="Horse, Breed, Age, Owner..."
              style={{ ...S.input, paddingLeft: 30, borderRadius: 20, width: 230, border: `1px solid ${C.cardInner}` }}
            />
          </div>
      ...
      <div style={{ fontSize: 10, letterSpacing: 3, textTransform: "uppercase", color: C.textFaint, marginBottom: 16 }}>
        {filtered.length} horses in care
      </div>
      ...
      {filtered.map((h) => (
        <HorseCard
          ...
          onEdit={() => setModal({ type: "edit", horse: h })}
          onDelete={() => setConfirm(h.id)}
